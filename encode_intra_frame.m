...
function encoded_frame = encode_intra_frame(frame,intra_type,blk_size)
 %decide on skipping intra frame coding
global intra_frame_coding;

if intra_frame_coding
    if(intra_type == 4)
       encoded_frame =  intra_4x4(frame,blk_size);
    else
       encoded_frame = intra_8x8(frame,blk_size);
    end
else
    encoded_frame = frame;
end
...
%blk_size = 16/8
function reconstructed = intra_4x4(frame,blk_size)
...
%using 3 modes for intra 8x8 mode
